...
public class PedidoProjectionHandler {
    ...
    public void handlePedidoCriado(PedidoCriado evento) {
        try {
            ...
            List<com.example.eventsourcing.application.readmodel.ItemPedido> itensReadModel = evento.itens().stream()
                    .map(itemEvent -> com.example.eventsourcing.application.readmodel.ItemPedido.from(itemEvent))
    ...
    private com.example.eventsourcing.application.readmodel.EnderecoEntrega convertEndereco(
            com.example.eventsourcing.domain.pedido.EnderecoEntrega domainEndereco) {

        if (domainEndereco == null) {
            return null;
        }

        return new com.example.eventsourcing.application.readmodel.EnderecoEntrega(
